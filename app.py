...
def get_klines(symbol):
    try:
        ...
        params = {"secid": secid, "fields1": "f1,f2,f3,f4,f5,f6", "fields2": "f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61", "klt": 101, "fqt": 0, "beg": "20240101", "end": "20260222", "lmt": 90}
        resp = requests.get(url, params=params, timeout=8)
        data = resp.json()
        if data.get("data") and data["data"].get("klines"):
            return data["data"]["klines"][-90:]
    except:
        pass
    return None
...
def calc_all(klines):
    ...
    ind['ma5上穿ma10'] = 1 if ind['ma5'] > ind['ma10'] and ind['ma10'] >= ind['ma20'] else 0
    ...
    ind['缩量'] = 1 if volumes[-1] < v5 * 0.6 else 0
    ...
    if len(closes) >= 14:
        ...
        ind['rsi金叉'] = 1 if ind['rsi'] < 40 and ind['rsi'] > 30 else 0
...
def score_stock_strict(ind):
    ...
    if wr_below_20 >= 3:  # 3个周期以上超卖
        score += 20
        confidence += 3
        reasons.append("WR多周期严重超卖")
    elif wr_below_20 >= 2:
        score += 15
        confidence += 2
    ...
    # === 严格信号判断 ===
    # 强烈买入: 分数>=40 且 置信度>=4
    if score >= 45 and confidence >= 4:
        signal = "强烈买入"
    elif score >= 35 and confidence >= 3:
        signal = "强烈买入"
...
def top_stocks():
    try:
        ...
        params = {"pn": 1, "pz": 8, "po": 1, "np": 1, "fltt": 2, "invt": 2, "fid": "f3", "fs": "m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23", "fields": "f12,f13,f14,f2,f3"}
        resp = requests.get(url, params=params, timeout=10)
        data = resp.json()
        stocks = data.get("data", {}).get("diff", [])[:6]
    except:
        stocks = []
...
def limitup():
    try:
        url = 'https://push2.eastmoney.com/api/qt/clist/get'
        params = {"pn": 1, "pz": 8, "po": 1, "np": 1, "fltt": 2, "invt": 2, "fid": "f3", "fs": "m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23", "fields": "f12,f13,f14,f3"}
        resp = requests.get(url, params=params, timeout=10)
        data = resp.json()
